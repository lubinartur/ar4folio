...
# CURSOR PROMPT — FIX STACK SECTION WIDTH (FULL CONTAINER)

Нужно исправить секцию «Стек / Инструменты», чтобы она визуально и структурно занимала всю ширину основного контейнера сайта, как и остальные секции.

## ❌ Текущая проблема
- Блок “Стек” выглядит уже, чем остальные секции страницы
- Контент визуально “утоплен” внутрь
- Нарушена общая сетка страницы → ощущение инородного блока

## ✅ Ожидаемое поведение
- Секция “Стек” должна строго соответствовать общей ширине layout’а сайта
- Заголовок и карточки должны выравниваться по тем же вертикальным направляющим, что и остальные секции
- Никакого отдельного max-width или лишнего wrapper’а

## 🔧 Что нужно сделать (проверь по порядку)
1. Найти компонент секции “Stack / Tools”
   - Скорее всего `StackSection`, `ToolsSection` или аналог
2. Проверить структуру:
   - У секции не должно быть собственного `max-width`
   - Убрать `max-width`, `margin: 0 auto`, `padding-inline`, если они дублируют layout
3. Секция должна использовать тот же `layout-container`, что и остальные блоки сайта. Например:
...
где `.container` — единый контейнер сайта, а не локальный.

4. Если внутри есть grid:
   - Grid должен растягиваться на 100% контейнера
   - Не использовать вложенные контейнеры с фиксированной шириной

## 📐 CSS требования
- `section.stack { width: 100%; }`
- Внутренний wrapper — тот же `.container`, что используется по всему сайту
- Не создавать новый `max-width` специально для этого блока

## Acceptance criteria
- Левая и правая колонки секции “Стек” совпадают по ширине с другими секциями
- Заголовок “Стек” выровнен по общей вертикали сайта
- Карточки доходят до тех же границ, что и карточки в других блоках
- При ресайзе нет “скачков” ширины относительно других секций

## ❗️Важно
Это layout-фикс, не редизайн.  
Ничего не менять в визуале карточек, отступах внутри карточек и типографике — только выравнивание секции по общей сетке.
